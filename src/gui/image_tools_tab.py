...
# Internal filename: 'src\\gui\\image_tools_tab.py'
...
from src.core.constants import REMBG_MODEL_FAMILIES, WAIFU2X_MODELS, SRMD_MODELS, VIDEO_EXTENSIONS, AUDIO_EXTENSIONS, IMAGE_RASTER_FORMATS, IMAGE_RAW_FORMATS, AI_FAMILY_HOLDER, AI_ENGINE_HOLDER, AI_MODEL_HOLDER
...
class ComparisonViewer(ctk.CTkCanvas):
    ...
    def load_images(self, img_original, img_result):
        ...
        self.delete('all')
        if not img_original or not img_result:
            return None
        else:
    ...
    def _on_zoom(self, event):
        if not self.img_after:
            ...
            if new_scale < 0.001 or new_scale > 100.0:
                return None
            else:
                self.scale = new_scale
                self.pan_x = mouse_x - img_x * self.scale
    ...
    def _on_left_drag(self, event):
        if not self.img_after:
            ...
            if self._drag_data['mode'] == 'pan':
                ...
                if self._drag_data['mode'] == 'slider':
...
class ImageToolsTab(ctk.CTkFrame):
    ...
    COMPATIBLE_EXTENSIONS = ('.png', '.jpg', '.jpeg', '.webp', '.bmp', '.tiff', '.tif', '.gif', '.avif', '.pdf', '.svg', '.eps', '.ai', '.ps', '.psd', '.tga', '.jp2', '.ico', '.cr2', '.dng', '.arw', '.nef', '.orf', '.rw2', '.sr2', '.raf', '.cr3', '.pef')
    def __init__(self, master, app, poppler_path=None, inkscape_path=None):
        ...
        self.image_processor = ImageProcessor(poppler_path=poppler_path, inkscape_service=self.app.inkscape_service, ffmpeg_path=self.app.ffmpeg_processor.ffmpeg_path)
        self.image_converter = ImageConverter(poppler_path=poppler_path, inkscape_service=self.app.inkscape_service, ffmpeg_processor=self.app.ffmpeg_processor)
    ...
    def refresh_theme(self):
        ...
        if hasattr(self, 'file_list_box'):
            ...
            try:
                self.file_list_box.configure(bg=listbox_bg, fg=listbox_fg, selectbackground=listbox_sel_bg, selectforeground=listbox_sel_fg, highlightbackground=listbox_border)
        ...
        frames_to_fix = ['format_master_frame', 'format_options_frame', 'resize_master_frame', 'canvas_master_frame', 'rembg_master_frame', 'upscale_master_frame', 'background_master_frame', 'output_frame', 'png_options_frame', 'jpg_options_frame', 'webp_options_frame', 'avif_options_frame', 'pdf_options_frame', 'video_options_frame']
    ...
    def _create_top_bar(self):
        ...
        self.paste_button = ctk.CTkButton(self.top_bar, text='Pegar', width=80, fg_color=self.DOWNLOAD_BTN_COLOR, hover_color=self.DOWNLOAD_BTN_HOVER, text_color=self.DOWNLOAD_BTN_TEXT, command=self._on_paste_list)
    ...
    def _create_viewer_panel(self):
        ...
        self.viewer_placeholder = ctk.CTkLabel(self.viewer_frame, text='Selecciona un archivo de la lista para previsualizarlo', text_color='gray')
    ...
    def _add_resolution_labels(self, original_size=None, result_size=None, is_vector=False):
        ...
        if original_size:
            ...
            if is_vector:
                text_orig += '\n(Escala de Preview)'
            res_orig = ctk.CTkLabel(self.viewer_frame, text=text_orig, fg_color=self.HUD_BG, text_color=self.HUD_TEXT, corner_radius=0, font=label_font, height=22, padx=8)
            res_orig.place(relx=0.98, rely=0.03, anchor='ne')
            res_orig.lift()
        if result_size:
            w, h = result_size
            res_res = ctk.CTkLabel(self.viewer_frame, text=f'Resultado: {w}x{h} px', fg_color=self.HUD_BG, text_color=self.HUD_TEXT, corner_radius=0, font=label_font, height=22, padx=8)
    ...
    def _create_left_panel(self):
        ...
        self.file_list_box = tkinter.Listbox(self.list_frame, bg=_lb_bg, fg=_lb_fg, selectbackground=_lb_sel, selectforeground=_lb_sft, borderwidth=0, highlightthickness=1, highlightbackground=_lb_bdr, font=('Segoe UI', 10), activestyle='none', selectmode='extended', exportselection=False, yscrollcommand=self.file_list_scrollbar_y.set, xscrollcommand=self.file_list_scrollbar_x.set)
        ...
        self.drag_hint_label = ctk.CTkLabel(self.list_frame, text='Arrastra archivos o carpetas aquí\no usa \'Importar Archivos\'', text_color='gray', fg_color=_lb_bg, font=ctk.CTkFont(size=12, weight='bold'))
        ...
        self.list_status_label.grid(row=4, column=0, padx=10, pady=(0, 5), sticky='w')
    def _create_right_panel(self):
        ...
        self.copy_result_button = ctk.CTkButton(self.title_frame, text='Copiar', width=60, state='disabled', command=self._copy_result_to_clipboard, fg_color=self.SECONDARY_BTN_COLOR, hover_color=self.SECONDARY_BTN_HOVER)
        ...
        self.options_frame = ctk.CTkScrollableFrame(self.right_panel, label_text='Opciones de Procesamiento')
        ...
        self.format_menu_frame.pack(fill='x', padx=5, pady=(5, 0))
        ...
        self.format_label.grid(row=0, column=0, padx=(5, 5), pady=5, sticky='w')
        self.export_formats = ['No Convertir', 'PNG', 'JPG', 'WEBP', 'AVIF', 'PDF', 'TIFF', 'ICO', 'BMP', '--- Video ---', '.mp4 (H.264)', '.mov (ProRes)', '.webm (VP9)', '.gif (Animado)']
        self.format_menu = ctk.CTkOptionMenu(self.format_menu_frame, values=self.export_formats, command=self._on_format_changed)
        self.format_menu.grid(row=0, column=1, padx=(0, 5), pady=5, sticky='ew')
        ...
        self.resize_checkbox = ctk.CTkCheckBox(self.resize_master_frame, text='Cambiar Tamaño (Escalar)', command=self._on_toggle_resize_frame)
        ...
        self.resize_preset_menu = ctk.CTkOptionMenu(self.resize_options_frame, values=['No escalar (Original)', '4K UHD (Máx: 3840×2160)', '2K QHD (Máx: 2560×1440)', '1080p FHD (Máx: 1920×1080)', '720p HD (Máx: 1280×720)', '480p SD (Máx: 854×480)', 'Personalizado...'], command=self._on_resize_preset_changed)
        ...
        self.interpolation_menu = ctk.CTkOptionMenu(self.interpolation_frame, values=list(INTERPOLATION_METHODS.keys()))
        ...
        self.resize_aspect_lock = ctk.CTkCheckBox(self.resize_custom_frame, text='', width=28)
        self.resize_aspect_lock.grid(row=0, column=2, padx=10, pady=5)
        self.resize_aspect_lock.select()
        Tooltip(self.resize_aspect_lock, text='Mantener Proporción', delay_ms=100)
        ...
        self.canvas_checkbox = ctk.CTkCheckBox(self.canvas_master_frame, text='Ajustar Canvas (Lienzo)', command=self._on_toggle_canvas_frame)
        ...
        self.canvas_option_menu = ctk.CTkOptionMenu(self.canvas_options_frame, values=CANVAS_OPTIONS, command=self._on_canvas_option_changed)
        ...
        Tooltip(self.canvas_margin_entry, text='Espacio que se añadirá en cada lado de la imagen.\nEjemplo: 100px = 100px arriba + 100px abajo + 100px izquierda + 100px derecha.\n\n• Margen Externo: El canvas crece (imagen + margen).\n• Margen Interno: La imagen se reduce (canvas - margen).', delay_ms=1000)
        ...
        self.canvas_position_menu = ctk.CTkOptionMenu(self.canvas_options_frame, values=CANVAS_POSITIONS)
        ...
        self.canvas_overflow_menu = ctk.CTkOptionMenu(self.canvas_overflow_frame, values=CANVAS_OVERFLOW_MODES)
        ...
        self.rembg_checkbox = ctk.CTkCheckBox(self.rembg_master_frame, text='Eliminar Fondo (IA)', command=self._on_toggle_rembg_frame)
        ...
        self.rembg_gpu_checkbox = ctk.CTkCheckBox(self.rembg_options_frame, text='Aceleración de Hardware (GPU)')
        ...
        self.rembg_family_menu = ctk.CTkOptionMenu(self.rembg_options_frame, values=[AI_ENGINE_HOLDER] + list(REMBG_MODEL_FAMILIES.keys()), command=self._on_rembg_family_change)
        ...
        self.rembg_model_menu = ctk.CTkOptionMenu(self.rembg_options_frame, values=[AI_MODEL_HOLDER], command=self._on_rembg_model_change)
        ...
        self.rembg_open_btn = ctk.CTkButton(self.rembg_actions_frame, text='Abrir', height=24, fg_color=self.SECONDARY_BTN_COLOR, hover_color=self.SECONDARY_BTN_HOVER, text_color=self.SECONDARY_BTN_TEXT, command=lambda: self._open_model_folder('rembg'))
        self.rembg_open_btn.grid(row=0, column=0, padx=(0, 5), sticky='ew')
        self.rembg_delete_btn = ctk.CTkButton(self.rembg_actions_frame, text='Borrar', height=24, fg_color=self.CANCEL_BTN_COLOR, hover_color=self.CANCEL_BTN_HOVER, text_color=self.CANCEL_BTN_TEXT, command=lambda: self._delete_current_model('rembg'))
        self.rembg_delete_btn.grid(row=0, column=1, padx=(5, 0), sticky='ew')
        ctk.CTkFrame(self.rembg_options_frame, height=1, fg_color=self.SEPARATOR_COLOR).grid(row=5, column=0, columnspan=2, sticky='ew', padx=10, pady=(8, 4))
        ...
        self.rembg_smooth_slider = ctk.CTkSlider(smooth_row, from_=0, to=20, number_of_steps=20, command=_on_smooth_change)
        ...
        self.rembg_expand_slider = ctk.CTkSlider(expand_row, from_=(-10), to=10, number_of_steps=20, command=_on_expand_change)
        ...
        self.upscale_checkbox = ctk.CTkCheckBox(self.upscale_master_frame, text='Reescalado con IA', command=self._on_toggle_upscale_frame)
        ...
        self.upscale_engine_menu = ctk.CTkOptionMenu(self.upscale_options_frame, values=[AI_ENGINE_HOLDER, 'Upscayl', 'Waifu2x', 'SRMD (Enfoque/Deblur)'], command=self._on_upscale_engine_change)
        ...
        self.upscale_model_menu = ctk.CTkOptionMenu(self.upscale_model_container, values=[AI_MODEL_HOLDER], command=self._on_upscale_model_change)
        self.upscale_model_menu.set(AI_MODEL_HOLDER)
        self.upscale_model_menu.grid(row=0, column=0, padx=(0, 5), pady=5, sticky='ew')
        self.upscale_add_custom_btn = ctk.CTkButton(self.upscale_model_container, text='+', width=30, height=28, fg_color=self.DOWNLOAD_BTN_COLOR, hover_color=self.DOWNLOAD_BTN_HOVER, text_color=self.DOWNLOAD_BTN_TEXT, font=ctk.CTkFont(size=16, weight='bold'), command=self._on_add_custom_model)
        ...
        self.upscale_scale_menu = ctk.CTkOptionMenu(self.upscale_options_frame, values=['2x', '3x', '4x'], width=70)
        self.upscale_scale_menu.set('2x')
        self.upscale_scale_menu.grid(row=2, column=1, padx=(0, 10), pady=5, sticky='w')
        ...
        self.upscale_tile_entry.grid(row=2, column=3, padx=(0, 10), pady=5, sticky='w')
        ...
        self.upscale_threads_menu = ctk.CTkOptionMenu(self.upscale_options_frame, values=['Automático', 'Seguro (Estabilidad)', 'Equilibrado', 'Máximo (Potente)'], width=150)
        ...
        self.upscale_denoise_menu = ctk.CTkOptionMenu(self.upscale_options_frame, values=['-1 (Ninguna)', '0 (Baja)', '1 (Media)', '2 (Alta)', '3 (Máxima)'])
        ...
        self.upscale_open_btn = ctk.CTkButton(self.upscale_actions_frame, text='Abrir', height=24, fg_color=self.SECONDARY_BTN_COLOR, hover_color=self.SECONDARY_BTN_HOVER, text_color=self.SECONDARY_BTN_TEXT, command=lambda: self._open_model_folder('upscale'))
        self.upscale_open_btn.grid(row=0, column=0, padx=(0, 5), sticky='ew')
        self.upscale_delete_btn = ctk.CTkButton(self.upscale_actions_frame, text='Borrar', height=24, fg_color=self.CANCEL_BTN_COLOR, hover_color=self.CANCEL_BTN_HOVER, text_color=self.CANCEL_BTN_TEXT, command=lambda: self._delete_current_model('upscale'))
        ...
        self.background_checkbox = ctk.CTkCheckBox(self.background_master_frame, text='Cambiar Fondo (Transparente)', command=self._on_toggle_background_frame)
        ...
        self.background_type_menu = ctk.CTkOptionMenu(self.background_options_frame, values=BACKGROUND_TYPES, command=self._on_background_type_changed)
        ...
        self.bg_color_button = ctk.CTkButton(self.bg_solid_frame, text='🎨', width=40, command=self._pick_solid_color)
        ...
        self.bg_gradient_color1_button = ctk.CTkButton(self.bg_gradient_frame, text='🎨', width=40, command=lambda: self._pick_gradient_color(1))
        ...
        self.bg_gradient_color2_button = ctk.CTkButton(self.bg_gradient_frame, text='🎨', width=40, command=lambda: self._pick_gradient_color(2))
        ...
        self.bg_gradient_direction_menu = ctk.CTkOptionMenu(self.bg_gradient_frame, values=GRADIENT_DIRECTIONS, width=200)
        ...
        self.bg_image_button = ctk.CTkButton(self.bg_image_frame, text='📁', width=40, command=self._select_background_image)
    ...
    def _create_bottom_panel(self):
        ...
        self.open_folder_button = ctk.CTkButton(line1_frame, text='📁', width=40, font=ctk.CTkFont(size=16), command=self._open_batch_output_folder, state='disabled')
        ...
        self.conflict_policy_menu = ctk.CTkOptionMenu(line2_frame, width=120, values=['Sobrescribir', 'Renombrar', 'Omitir'])
        ...
        self.start_process_button = ctk.CTkButton(line2_frame, text='Iniciar Proceso', state='disabled', command=self._on_start_process, fg_color=self.PROCESS_BTN_COLOR, hover_color=self.PROCESS_BTN_HOVER, text_color_disabled=self.DISABLED_TEXT_COLOR, width=140)
    ...
    def _on_format_changed(self, selected_format):
        ...
        if selected_format == 'No Convertir' or is_video_format:
            ...
            if selected_format in FORMATS_WITH_TRANSPARENCY:
                show_background_module = True
    ...
    def _create_slider_with_label(self, parent, text, min_val, max_val, default_val, step=1):
        ...
        slider = ctk.CTkSlider(frame, from_=min_val, to=max_val, number_of_steps=(max_val - min_val) // step if step != 0 else 100, command=slider_callback)
        slider.set(default_val)
        slider_callback(default_val)
    ...
    def _create_png_options(self):
        ...
        self.png_transparency.select()
        self.png_compression_slider, self.png_compression_label = self._create_slider_with_label(parent=png_frame, text='Compresión (0-9):', min_val=0, max_val=9, default_val=6, step=1)
    ...
    def _create_jpg_options(self):
        ...
        self.jpg_quality_slider, self.jpg_quality_label = self._create_slider_with_label(parent=jpg_frame, text='Calidad (1-100):', min_val=1, max_val=100, default_val=90, step=1)
        ...
        self.jpg_subsampling = ctk.CTkOptionMenu(sub_frame, values=['4:2:0 (Estándar)', '4:2:2 (Alta)', '4:4:4 (Máxima)'], width=200)
        ...
        self.option_frames['JPEG'] = jpg_frame
    def _create_webp_options(self):
        ...
        self.webp_lossless = ctk.CTkCheckBox(webp_frame, text='Compresión sin Pérdida (Lossless)', command=self._toggle_webp_quality)
        ...
        self.webp_quality_slider, self.webp_quality_label = self._create_slider_with_label(parent=self.webp_quality_frame, text='Calidad (1-100):', min_val=1, max_val=100, default_val=90, step=1)
        self.webp_transparency = ctk.CTkCheckBox(webp_frame, text='Mantener Transparencia', command=lambda: self._on_format_changed(self.format_menu.get()))
    ...
    def _create_avif_options(self):
        ...
        self.avif_lossless = ctk.CTkCheckBox(avif_frame, text='Sin Pérdida (Lossless)', command=self._toggle_avif_quality)
        ...
        self.avif_quality_slider, _ = self._create_slider_with_label(parent=self.avif_quality_frame, text='Calidad (1-100):', min_val=1, max_val=100, default_val=80, step=1)
        self.avif_speed_slider, _ = self._create_slider_with_label(parent=avif_frame, text='Velocidad (0-10):', min_val=0, max_val=10, default_val=6, step=1)
        Tooltip(self.avif_speed_slider, 'Compresión: 0=Lento/Mejor, 10=Rápido/Peor. Default: 6', delay_ms=1000)
        self.avif_transparency = ctk.CTkCheckBox(avif_frame, text='Mantener Transparencia', command=lambda: self._on_format_changed(self.format_menu.get()))
    ...
    def _on_canvas_option_changed(self, selection):
        ...
        if selection == 'Sin ajuste':
            ...
            if selection in ['Añadir Margen Externo', 'Añadir Margen Interno']:
                self.canvas_margin_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky='ew')
            else:
                if selection == 'Personalizado...':
                    ...
                    if selection in CANVAS_PRESET_SIZES:
    ...
    def _on_background_type_changed(self, selection):
        ...
        if selection == 'Color Sólido':
            ...
            if selection == 'Degradado':
                self.bg_gradient_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky='ew')
            else:
                if selection == 'Imagen de Fondo':
                    self.bg_image_frame.grid(row=1, column=0, columnspan=2, padx=10, pady=5, sticky='ew')
    def _pick_solid_color(self):
        ...
        try:
            color = colorchooser.askcolor(
                ...
                initialcolor=self.bg_color_entry.get()
            )
    ...
    def _pick_gradient_color(self, color_num):
        ...
        try:
            color = colorchooser.askcolor(
                title=f'Seleccionar Color {color_num}',
    ...
    def _select_background_image(self):
        ...
        from customtkinter import filedialog
        filepath = filedialog.askopenfilename(title='Seleccionar Imagen de Fondo', filetypes=[('Imágenes', '*.png *.jpg *.jpeg *.webp *.bmp'), ('Todos los archivos', '*.*')])
        self.app.lift()
    ...
    def _create_pdf_options(self):
        ...
        self.pdf_combine = ctk.CTkCheckBox(pdf_frame, text='Combinar todos en un solo PDF', command=self._toggle_pdf_title_entry)
        ...
        self.pdf_combined_title_entry = ctk.CTkEntry(self.pdf_title_frame, placeholder_text='combined_output')
    ...
    def _create_tiff_options(self):
        ...
        self.tiff_compression = ctk.CTkOptionMenu(comp_frame, values=['Ninguna', 'LZW (Recomendada)', 'Deflate (ZIP)', 'PackBits'], width=200)
        ...
        self.tiff_transparency = ctk.CTkCheckBox(tiff_frame, text='Mantener Transparencia', command=lambda: self._on_format_changed(self.format_menu.get()))
    ...
    def _on_upscale_model_change(self, selected_model_friendly, engine=None, silent=False):
        ...
        if self.upscale_checkbox.get() != 1:
            ...
            if selected_model_friendly == AI_MODEL_HOLDER:
                            ...
                            if user_response:
                                ...
                                def download_thread():
                                    def progress_cb(text, val):
                                        self.app.ui_update_queue.put((lambda t=text: self.upscale_status_label.configure(text=t), ()))
                                    success = check_and_download_upscaling_tools(progress_cb, target_tool=engine_key)
                                    if success:
                                        self.app.ui_update_queue.put((lambda: self._on_upscale_engine_change(engine, silent=True), ()))
                                    else:
                                        self.app.ui_update_queue.put((lambda: self.upscale_status_label.configure(text='❌ Error descarga', text_color='red'), ()))
                ...
                if not valid_scales:
                    print('⚠️ ADVERTENCIA: No se encontraron escalas válidas. Usando fallback [\'4x\']')
                    valid_scales = ['4x']
    ...
    def _on_resize_preset_changed(self, selection):
        ...
        preset_map = {'4K UHD (Máx: 3840×2160)': (3840, 2160), '2K QHD (Máx: 2560×1440)': (2560, 1440), '1080p FHD (Máx: 1920×1080)': (1920, 1080), '720p HD (Máx: 1280×720)': (1280, 720), '480p SD (Máx: 854×480)': (854, 480), 'No escalar (Original)': None}
        if selection == 'Personalizado...':
            self.resize_custom_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=5, sticky='ew')
            return
        else:
            ...
            if selection in preset_map:
                ...
                if dimensions:
                    ...
                    self.resize_aspect_lock.select()
                    self.resize_aspect_lock.configure(state='disabled')
                    print(f'Preset aplicado: {width}×{height} (Proporción forzada)')
                else:
                    self.resize_width_entry.delete(0, 'end')
                    self.resize_height_entry.delete(0, 'end')
                    self.resize_aspect_lock.configure(state='normal')
    def _create_video_options(self):
        ...
        self.video_filename_entry = ctk.CTkEntry(self.video_name_frame, placeholder_text='Opcional (Auto: Primera Imagen)')
        ...
        self.video_resolution_menu = ctk.CTkOptionMenu(self.res_frame, values=['Usar la primera (Auto)', '1920x1080 (1080p)'], command=self._on_video_resolution_changed)
        ...
        self.video_fit_mode_menu = ctk.CTkOptionMenu(fit_frame, values=['Mantener Tamaño Original', 'Ajustar al Fotograma (Barras)', 'Ajustar al Marco (Recortar)'])
    ...
    def _process_batch_as_video(self, output_dir, options):
        ...
        try:
            ...
            self.start_process_button.configure(state='normal', text='Cancelar', fg_color=self.CANCEL_BTN_COLOR, hover_color=self.CANCEL_BTN_HOVER)
            self.import_button.configure(state='disabled')
            threading.Thread(target=self._video_thread_target, args=(output_dir, options), daemon=True).start()
    def _video_thread_target(self, output_dir, options, cancel_event):
        ...
        def progress_callback(phase, progress_pct, message):
            if cancel_event.is_set():
                ...
                if phase == 'Standardizing':
                    ...
                    self.app.after(0, lambda: self.progress_bar.set(progress_pct / 100.0 * 0.5))
                else:
                    if phase == 'Encoding':
                        self.app.after(0, lambda: self.progress_label.configure(text=f'[Fase 2/2] Codificando: {message}'))
                        self.app.after(0, lambda: self.progress_bar.set(0.5 + progress_pct / 100.0 * 0.5))
        ...
        try:
            ...
            final_video_path = self.image_converter.create_video_from_images(file_data_list=clean_file_list, output_path=output_path, options=options, progress_callback=progress_callback, cancellation_event=cancel_event)
            ...
            self.app.after(0, lambda: self.start_process_button.configure(state='normal', text='Iniciar Proceso', fg_color=self.PROCESS_BTN_COLOR, hover_color=self.PROCESS_BTN_HOVER))
            if hasattr(self, 'import_button'):
    ...
    def _initialize_ui_settings(self):
        ...
        try:
            saved_format = settings.get('format', 'PNG')
            self.format_menu.set(saved_format)
            self._on_format_changed(saved_format)
            res = settings.get('resize', {})
            if res.get('enabled'):
                self.resize_checkbox.select()
            else:
                self.resize_checkbox.deselect()
            if res.get('preset'):
                self.resize_preset_menu.set(res['preset'])
            if res.get('width'):
                self.resize_width_entry.delete(0, 'end')
            ...
            if res.get('lock'):
                ...
                self.resize_aspect_lock.deselect()
            if res.get('interpolation'):
                self.interpolation_menu.set(res['interpolation'])
            self._on_toggle_resize_frame()
            if res.get('preset'):
                self._on_resize_preset_changed(res.get('preset'))
            can = settings.get('canvas', {})
            if can.get('enabled'):
                self.canvas_checkbox.select()
            else:
                self.canvas_checkbox.deselect()
            if can.get('option'):
                self.canvas_option_menu.set(can['option'])
            if can.get('margin'):
            ...
            if can.get('position'):
                self.canvas_position_menu.set(can['position'])
            if can.get('overflow'):
                self.canvas_overflow_menu.set(can['overflow'])
            self._on_toggle_canvas_frame()
            bg = settings.get('background', {})
            if bg.get('enabled'):
                self.background_checkbox.select()
            else:
                self.background_checkbox.deselect()
            if bg.get('type'):
                self.background_type_menu.set(bg['type'])
            if bg.get('color'):
                self.bg_color_entry.delete(0, 'end')
            ...
            if bg.get('direction'):
                self.bg_gradient_direction_menu.set(bg['direction'])
            ...
            if settings.get('upscale_enabled'):
                self.upscale_checkbox.select()
            else:
                self.upscale_checkbox.deselect()
            ...
            if settings.get('upscale_scale'):
                self.upscale_scale_menu.set(settings['upscale_scale'])
            if settings.get('upscale_denoise'):
                self.upscale_denoise_menu.set(settings['upscale_denoise'])
            if settings.get('upscale_tile'):
                self.upscale_tile_entry.delete(0, 'end')
                self.upscale_tile_entry.insert(0, settings['upscale_tile'])
            if settings.get('upscale_tta'):
                self.upscale_tta_check.select()
            else:
                self.upscale_tta_check.deselect()
    ...
    def save_settings(self):
        ...
        current_settings = {
            'format': {
                'enabled': self.format_menu.get(),
                'preset': self.resize_checkbox.get(),
                'width': self.resize_preset_menu.get(),
                'height': self.resize_width_entry.get(),
                'lock': self.resize_height_entry.get(),
                'interpolation': self.interpolation_menu.get()
            },
            'resize': {
                'enabled': self.canvas_checkbox.get(),
                'type': self.canvas_option_menu.get(),
                'color': self.canvas_margin_entry.get(),
                'grad_c1': self.canvas_width_entry.get(),
                'grad_c2': self.canvas_height_entry.get(),
                'direction': self.bg_gradient_direction_menu.get()
            },
            'canvas': {
                'enabled': self.upscale_checkbox.get() == 1,
                'type': self.upscale_scale_menu.get(),
                'color': self.upscale_denoise_menu.get(),
                'grad_c1': self.upscale_tile_entry.get(),
                'grad_c2': self.upscale_tta_check.get(),
                'direction': ''  # Reemplazar con el menú correspondiente si existe
            }
        }
    ...
    def _on_analyze_url(self):
        ...
        if url and self.is_analyzing_url:
            return None
        else:
    ...
    def _get_thumbnail_from_url(self, url):
        ...
        # Attempt to load cookies from saved settings
        try:
        ...
        try:
            ...
            if not thumbnail:
                return (None, 'No se encontró miniatura en la información.')
            return (thumbnail, None)  # Success
    ...
    def _analyze_url_thread(self, url):
        ...
        try:
            ...
            if url.lower().endswith(direct_image_exts):
                ...
                print('INFO: URL no directa. Usando yt-dlp para buscar miniatura...')
    ...
    def _on_image_list_drop(self, event):
        ...
        try:
            ...
            threading.Thread(target=self._scan_and_import_dropped_paths, args=(paths,), daemon=True).start()
    ...
    def _on_import_files(self):
        ...
        filetypes = [('Archivos de Imagen Compatibles', ' '.join(self.COMPATIBLE_EXTENSIONS)), ('Todos los archivos', '*.*')]
        filepaths = filedialog.askopenfilenames(title='Importar Archivos de Imagen', filetypes=filetypes)
        self.app.lift()
        self.app.focus_force()
        if filepaths:
    ...
    def _on_import_folder(self):
        ...
        folder_path = filedialog.askdirectory(title='Importar Carpeta (se escaneará recursivamente)')
        self.app.lift()
        self.app.focus_force()
        if not folder_path:
            return
            ...
            threading.Thread(target=self._search_folder_thread, args=(folder_path,), daemon=True).start()
    ...
    def _process_imported_files(self, filepaths):
        ...
        for path in filepaths:
            ...
            range_string = dialog.get_result()
            if range_string:
    ...
    def _on_paste_list(self):
        ...
        # Si no hay imagen, intentar pegar texto (rutas de archivo)
        try:
            content = self.clipboard_get()
            filepaths = [path.strip() for path in content.splitlines() if path.strip()]
            valid_files = [
                path for path in filepaths
                if os.path.exists(path) and path.lower().endswith(self.COMPATIBLE_EXTENSIONS)
            ]
            if valid_files:
    ...
    def _on_file_select(self, event=None):
        ...
        try:
            ...
            saved_title = item_data[3] if len(item_data) > 3 else None

            # --- Store output path and update copy button ---
            ...
            if hasattr(self, 'copy_result_button'):
                if output_path and os.path.exists(output_path):
                    ...
                    if ext in ['.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.webp', '.ico'] and os.path.isfile(output_path):
                        self.copy_result_button.configure(state='normal', fg_color=self.PROCESS_BTN_COLOR, hover_color=self.PROCESS_BTN_HOVER)
                    else:
                        self.copy_result_button.configure(state='disabled')
                else:
                    self.copy_result_button.configure(state='disabled')

            # --- Display preview and set title ---
            if output_path and os.path.exists(output_path):
                # Output exists → show comparison and use saved title if available
                ...
                # No output yet → show input preview and generate title from input
                if saved_title:
                    title_to_show = saved_title
                else:
            ...
            # --- Handle thumbnail loading via queue ---
            ...
            # Not in cache: clear the queue and enqueue new request
            try:
            ...
            # Show loading message while thumbnail is being generated
            ...
            print(f'ERROR: No se pudo seleccionar el ítem en el índice {first_index if "first_index" in locals() else "?"}: {e}')
            self._display_thumbnail_in_viewer(None, None)
    ...
    def _on_start_process(self):
        ...
        # --- Check for Ghostscript (gs_exe) ---
        if hasattr(self.image_converter, 'gs_exe') and self.image_converter.gs_exe:
            try:
                import stat
                existe = os.path.exists(self.image_converter.gs_exe)
                if existe:
                    print(f'   ✅ gs_exe encontrado: {self.image_converter.gs_exe}')
            except Exception as e:
                print(f'   ⚠️ Error al verificar gs_exe: {e}')
        else:
        ...
        # --- Handle 'No Convertir' with raster operations on vectors ---
        if self.format_menu.get() == 'No Convertir':
            is_raster_op = self.resize_checkbox.get() == 1 or self.canvas_checkbox.get() == 1 or self.background_checkbox.get() == 1
            if is_raster_op:
                has_vectors = False
        ...
        # --- Validate resize dimensions if enabled ---
        if self.resize_checkbox.get():
            try:
                ...
                if width <= 0 or height <= 0:
                    Tooltip.hide_all()
                    messagebox.showwarning('Dimensiones inválidas', 'El ancho y alto deben ser mayores a 0.')
                    return
                ...
                Tooltip.hide_all()
                messagebox.showwarning('Dimensiones inválidas', 'Ingresa valores numéricos válidos para ancho y alto.')
                return

        # --- Validate canvas dimensions if enabled ---
            ...
            if canvas_option == 'Personalizado...':
                try:
                    ...
                    if canvas_width <= 0 or canvas_height <= 0:
                        Tooltip.hide_all()
                        messagebox.showwarning('Dimensiones inválidas', 'El ancho y alto del canvas deben ser mayores a 0.')
                        return
                    ...
                    Tooltip.hide_all()
                    messagebox.showwarning('Dimensiones inválidas', 'Ingresa valores numéricos válidos para el canvas.')
                    return

            if canvas_option in ['Añadir Margen Externo', 'Añadir Margen Interno']:
        ...
        # --- Validate video export options if needed ---
        ...
        if is_video_export:
            try:
                ...
                duration = int(options.get('video_frame_duration', '3'))
                if fps <= 0 or duration <= 0:
                    raise ValueError('FPS/Duración debe ser > 0')
                if options.get('video_resolution') == 'Personalizado...':
                    width = int(options.get('video_custom_width', '1920'))
                    height = int(options.get('video_custom_height', '1080'))
                    if width <= 0 or height <= 0:
                        raise ValueError('Dimensiones inválidas')
                ...
                Tooltip.hide_all()
                messagebox.showerror('Valores Inválidos', f'Error en la configuración de vídeo:\n{e}')
                return
        ...
        threading.Thread(
            ...
            args=(output_dir, self.cancel_event),
            daemon=True
        ).start()
    def _copy_result_to_clipboard(self):
        ...
        try:
            ...
            class DROPFILES(ctypes.Structure):
                _fields_ = [('pFiles', ctypes.c_uint32), ('pt_x', ctypes.c_long), ('pt_y', ctypes.c_long), ('fNC', ctypes.c_int), ('fWide', ctypes.c_int)]
            files_list = [file_path]
            file_data = ('\x00'.join(files_list) + '\x00\x00').encode('utf-16-le')
            ...
            if not hGlobal:
                raise MemoryError('GlobalAlloc falló.')
            else:
    ...
    def _process_files_thread(self, output_dir, cancel_event):
        ...
        # Handle video export separately (assumed to be implemented)
        if is_video_export:
            self._video_thread_target(output_dir, options, cancel_event)
            return

        # --- Prepare AI sessions and Inkscape ---
        ...
        # Reset status colors (if not process-only-new)
        ...
        for i, item_data in enumerate(self.file_list_data):
            ...
            # Skip if already processed (and option is enabled)
            ...
            # --- Callback for per-file progress ---
            ...
            # --- Perform conversion (wrapped in try/except) ---
            try:
                ...
                if success:
                    ...
                    # Update file_list_data with output path
                ...
                elif 'MAX_TEXT_CHUNK' in error_msg:
        ...
        # --- After loop: combine PDFs if needed ---
        if (not cancel_event.is_set() and
            options['format'] == 'PDF' and
            options.get('pdf_combine', False) and
            len(generated_pdfs) > 1):
            self.app.after(0, lambda: self.progress_label.configure(text=f'Combinando {len(generated_pdfs)} PDFs...'))
            ...
            if self.image_converter.combine_pdfs(generated_pdfs, combined_pdf_path):
                ...
                # Remove the individual PDFs from the success list and disk
        ...
        # Import to integrations if not cancelled
        if not cancel_event.is_set():
            print('DEBUG: Proceso finalizado. Programando importación.')
        ...
        # --- Cleanup (Inkscape and AI sessions) ---
        try:
            if inkscape_active and self.app.inkscape_service:
                self.app.inkscape_service.stop_session()
        except Exception as e:
            print(f'ADVERTENCIA: Error al detener Inkscape: {e}')
    ...
    def _gather_conversion_options(self):
        """
        Recopila todas las opciones de conversión desde la interfaz de usuario.
        ...
        # --- Format settings ---
        options['format'] = self.format_menu.get()
        options['preset'] = self.resize_checkbox.get() == 1
        options['interpolation'] = self.interpolation_menu.get()

        # --- Resize settings ---
        options['resize_enabled'] = self.resize_checkbox.get() == 1
        options['resize_width'] = self.resize_width_entry.get()
        options['resize_height'] = self.resize_height_entry.get()
        options['resize_preset'] = self.resize_preset_menu.get()

        # --- Canvas settings ---
        options['canvas_enabled'] = self.canvas_checkbox.get() == 1
        options['canvas_type'] = self.canvas_option_menu.get()
        options['canvas_width'] = self.canvas_width_entry.get()
        options['canvas_height'] = self.canvas_height_entry.get()
        options['canvas_margin'] = self.canvas_margin_entry.get()
        options['canvas_color'] = self.canvas_color_entry.get() if hasattr(self, 'canvas_color_entry') else '#000000'
        options['canvas_position'] = self.canvas_position_menu.get() if hasattr(self, 'canvas_position_menu') else 'Centro'
        options['canvas_overflow'] = self.canvas_overflow_menu.get() if hasattr(self, 'canvas_overflow_menu') else 'Centrar (puede recortar)'
        options['gradient_color1'] = self.bg_gradient_color1_entry.get() if hasattr(self, 'bg_gradient_color1_entry') else '#FF0000'
        options['gradient_color2'] = self.bg_gradient_color2_entry.get() if hasattr(self, 'bg_gradient_color2_entry') else '#0000FF'
        options['gradient_direction'] = self.bg_gradient_direction_menu.get() if hasattr(self, 'bg_gradient_direction_menu') else 'horizontal'

        # --- Background settings ---
        options['background_enabled'] = self.background_checkbox.get() == 1 if hasattr(self, 'background_checkbox') else False
        options['background_type'] = self.background_type_menu.get() if hasattr(self, 'background_type_menu') else 'Color Sólido'
        options['background_color'] = self.bg_color_entry.get() if hasattr(self, 'bg_color_entry') else '#FFFFFF'

        # --- Upscaling settings ---
        options['upscale_enabled'] = self.upscale_checkbox.get() == 1 if hasattr(self, 'upscale_checkbox') else False
        options['upscale_scale'] = self.upscale_scale_menu.get() if hasattr(self, 'upscale_scale_menu') else '2x'
        options['upscale_denoise'] = self.upscale_denoise_menu.get() if hasattr(self, 'upscale_denoise_menu') else 'None'
        options['upscale_tile'] = self.upscale_tile_entry.get() if hasattr(self, 'upscale_tile_entry') else '0'
        options['upscale_tta'] = self.upscale_tta_check.get() == 1 if hasattr(self, 'upscale_tta_check') else False

        # --- Rembg (background removal) settings ---
        options['rembg_enabled'] = self.rembg_checkbox.get() == 1 if hasattr(self, 'rembg_checkbox') else False
        if hasattr(self, 'rembg_family_menu') and hasattr(self, 'rembg_model_menu'):
            selected_family = self.rembg_family_menu.get()
            selected_model_label = self.rembg_model_menu.get()
            # Resolve real model name
            from src.core.constants import REMBG_MODEL_FAMILIES  # ensure import
            real_model_name = 'u2netp'  # default
            ...
            options['rembg_model'] = real_model_name
        else:
            options['rembg_model'] = 'u2netp'

        # --- Video export settings (if format is video) ---
        if options['format'].startswith('.'):  # e.g., '.mp4', '.webm'
            options['video_fps'] = self.video_fps_entry.get() if hasattr(self, 'video_fps_entry') else '30'
            options['video_frame_duration'] = self.video_frame_duration_entry.get() if hasattr(self, 'video_frame_duration_entry') else '3'
            options['video_codec'] = self.video_codec_menu.get() if hasattr(self, 'video_codec_menu') else 'libx264'
            # Resolution
            ...
            if vid_res_selection == 'Personalizado...':
                options['video_resolution'] = 'Personalizado...'
                options['video_custom_width'] = self.video_custom_width_entry.get() if hasattr(self, 'video_custom_width_entry') else '1920'
                options['video_custom_height'] = self.video_custom_height_entry.get() if hasattr(self, 'video_custom_height_entry') else '1080'
            elif vid_res_selection.startswith('Usar la primera'):
                options['video_resolution'] = 'auto'
                # Use dimensions of first image (will be resolved later)
                options['video_custom_width'] = 'auto'
                options['video_custom_height'] = 'auto'
            else:
                # Try to parse "WxH" from string
                try:
                    parts = vid_res_selection.split(' ')[0]
                    w, h = parts.split('x')
                    options['video_resolution'] = 'custom'
                    options['video_custom_width'] = w
                    options['video_custom_height'] = h
                except:
                    options['video_resolution'] = 'custom'
                    options['video_custom_width'] = '1920'
                    options['video_custom_height'] = '1080'
        else:
            # Not video: set defaults
            options['video_fps'] = '30'
            options['video_frame_duration'] = '3'
            options['video_resolution'] = 'none'
            options['video_custom_width'] = '1920'
            options['video_custom_height'] = '1080'

        # --- PDF combine settings ---
        options['pdf_combine'] = self.pdf_combine_checkbox.get() == 1 if hasattr(self, 'pdf_combine_checkbox') else False
        options['pdf_combined_title'] = self.pdf_combined_title_entry.get() if hasattr(self, 'pdf_combined_title_entry') else 'combined_output'

        # --- Conflict policy ---
        options['conflict_policy'] = self.conflict_policy_menu.get() if hasattr(self, 'conflict_policy_menu') else 'ask'

        # --- Other ---
        options['output_dir'] = self.output_path_entry.get()
    ...
    def _get_current_pdf_transparency_flag(self, selected_format):
        ...
        attr_map = {'PNG': 'png_pdf_transparent', 'WEBP': 'webp_pdf_transparent', 'AVIF': 'avif_pdf_transparent', 'TIFF': 'tiff_pdf_transparent', 'ICO': 'ico_pdf_transparent'}
    ...
    def _update_video_resolution_menu_options(self):
        ...
        base_options = ['1920x1080 (1080p)', '1280x720 (720p)', '3840x2160 (4K UHD)', 'Personalizado...']
    ...
    def _get_output_filename(self, input_path, options, page_num=None, custom_title=None):
        ...
        if output_format_str == 'No Convertir':
            ...
            from src.core.constants import IMAGE_INPUT_FORMATS
            if input_ext in IMAGE_INPUT_FORMATS:
                extension = 'png'
            else:
    ...
    def _import_to_integrations(self, processed_filepaths: list):
        ...
        files_to_import = [fp for fp in processed_filepaths if os.path.isfile(fp)]
    ...
    def _set_item_status_color(self, index, status):
        ...
        colors = {'success': '#76E068', 'skipped': '#FFD700', 'error': '#FF5252', 'pending': 'white'}
    ...
    def _add_files_to_list(self, file_tuples: list):
        ...
        for file_path, page_num in file_tuples:
            is_duplicate = any((item[0] == file_path and item[1] == page_num for item in self.file_list_data))
            if not is_duplicate:
    ...
    def _show_viewer_error(self, message):
        ...
        ctk.CTkLabel(error_frame, text='⚠️', font=('Arial', 30)).pack(pady=(10, 0))
        ctk.CTkLabel(error_frame, text=message, text_color='#FF9500', font=('Arial', 13, 'bold'), wraplength=350).pack(padx=20, pady=10)
    def _display_thumbnail_in_viewer(self, pil_image, original_filepath, is_loading=False, loading_message=None):
        ...
        if is_loading or loading_message:
            ...
            self.viewer_placeholder = ctk.CTkLabel(self.viewer_frame, text=msg, text_color='gray')
            self.viewer_placeholder.place(relx=0.5, rely=0.5, anchor='center')
        else:
            if pil_image or (original_filepath and os.path.exists(original_filepath)):
                ...
                try:
                    if original_filepath and os.path.exists(original_filepath) and is_raster_compatible:
                        ...
                        if pil_image:
                            print('DEBUG: Visor cargando Vector/Miniatura desde memoria')
                    ...
                    if 'decompression bomb' in error_msg.lower():
                        ...
                        if 'MemoryError' in error_msg or 'allocat' in error_msg.lower():
    ...
    def _start_thumbnail_worker(self):
        ...
        if self.active_thumbnail_thread and self.active_thumbnail_thread.is_alive():
                return
        self.active_thumbnail_thread = threading.Thread(target=self._thumbnail_worker_loop, daemon=True)
    ...
    def _thumbnail_worker_loop(self):
        ...
        while not getattr(self, 'stop_thumbnail_worker', False):
            try:
                # Wait for a thumbnail request (with timeout to check stop flag)
                filepath, page_num = self.thumbnail_queue.get(timeout=0.5)
            except queue.Empty:
                continue  # No items, loop again

            # Check if we should stop
            if getattr(self, 'stop_thumbnail_worker', False):
                break

            cache_key = f'{filepath}::{page_num}'

            # If the user already moved to another file, discard this thumbnail
            if cache_key != getattr(self, 'last_preview_path', ''):
                continue

            # Determine thumbnail size from viewer frame
            try:
            ...
            # Generate the thumbnail (may raise exceptions)
            try:
            ...
            # Double‑check that this thumbnail is still wanted (user may have moved on)
            if cache_key != getattr(self, 'last_preview_path', ''):
                print(f'DEBUG: Miniatura de \'{os.path.basename(filepath)}\' (pág. {page_num}) descartada después de generar')
                continue

            if pil_image:
                # Create CTkImage and store in cache
                ctk_image = ctk.CTkImage(
                    light_image=pil_image,
                    dark_image=pil_image,
                    size=(pil_image.width, pil_image.height)
                )
                cache_data = {'ctk': ctk_image, 'pil': pil_image}
        ...
        if original_filepath != self.last_preview_path:
            ...
            if os.path.exists(real_path) and os.path.isfile(real_path) and is_standard_raster:
                ...
                if isinstance(cache_data, dict) and 'pil' in cache_data:
                    ...
                    if original_filepath in self.thumbnail_cache:
        ...
        if category == 'rembg':
            ...
            if category == 'upscale':
                ...
                if engine == 'Upscayl':
                    ...
                    if engine == 'Waifu2x' and model_name in WAIFU2X_MODELS:
                            return (None, None)
    ...
    def _open_model_folder(self, category):
        ...
        if category == 'rembg':
            ...
            if category == 'upscale':
                engine = self.upscale_engine_menu.get()
                folder_map = {'Upscayl': 'upscayl', 'Real-ESRGAN': 'realesrgan', 'Waifu2x': 'waifu2x', 'RealSR': 'realsr', 'SRMD': 'srmd'}
        ...
        if category == 'rembg':
            ...
            if path:
                if os.path.exists(path):
                    ...
                    Tooltip.hide_all()
                    confirm = messagebox.askyesno('Confirmar Eliminación', f'¿Estás seguro de que deseas eliminar el modelo \'{model_name}\'?\n\nLiberarás {size_mb:.1f} MB de espacio.')
                    if confirm:
                        try:
                            os.remove(path)
            ...
            if category == 'upscale':
                ...
                if target_path and os.path.exists(target_path):
                    ...
                    Tooltip.hide_all()
                    confirm = messagebox.askyesno('Desinstalar Motor', f'¿Estás seguro de que deseas eliminar el motor \'{engine}\' completo?\n\nEsto borrará la carpeta entera y todos sus modelos.\nEspacio liberado: {size_mb:.1f} MB\n\nTendrás que volver a descargarlo para usarlo.')
                    if confirm:
                        try:
                    ...
                    if is_installed:
                        ...
                        if not silent:
                            ...
                            if family == 'RMBG 2.0 (BriaAI)' and 'danielgatis' not in model_info['url']:
                                ...
                                size_str = format_size(file_size)
                                Tooltip.hide_all()
                                user_response = messagebox.askyesno('Descargar Modelo IA', f'El modelo \'{selected_model}\' no está instalado.\n\nTamaño de descarga: {size_str}\n\n¿Deseas descargarlo ahora?')
                                if user_response:
                                    self.rembg_status_label.configure(text='Iniciando descarga...', text_color='#52a2f2')
                                    threading.Thread(target=self._download_rembg_model_thread, args=(model_info, file_path), daemon=True).start()
                                else:
    ...
    def _download_rembg_model_thread(self, model_info, file_path):
        ...
        try:
            ...
            # Verify file size
            if total_length > 0 and os.path.getsize(temp_path) != total_length:
            ...
            # Always close the session and re-enable UI elements
            if session:
                try:
                    session.close()
                except Exception:
    ...
    def _start_comparison_viewer(self, input_path, output_path, page_num):
        ...
        # Outer try: catch any error during the whole setup
        try:
            # --- 1. Get real original size (for label) ---
            ...
            if not img_before:
                ...
                try:
                    ...
                    print(f'ERROR al cargar imagen original: {e}')
                    self._show_viewer_error(f'No se pudo cargar la imagen original:\n{e}')
                    return

                # If we successfully loaded img_before, cache it
                if img_before:
                    if not real_original_size:
                        real_original_size = img_before.size
                    # Optionally downsize for memory
            ...
            # Fallback: if we couldn't load original but have result, use result as original
            ...
            if not img_after:
                ...
                self._show_viewer_error('No se pudo cargar la imagen de resultado.')
                return

            # --- 4. Align sizes if needed ---
            ...
            # --- 5. Create and configure ComparisonViewer ---
            _vbg = self._resolve_color(self.VIEWER_BG)
            ...
            import traceback
            traceback.print_exc()
            self._show_viewer_error(f'Error al iniciar la comparación:\n{e}')
    def _show_comparison_instructions(self):
        """Muestra una etiqueta temporal sobre cómo usar el visor."""
        info_label = ctk.CTkLabel(self.viewer_frame, text='Rueda: Zoom | Clic + Arrastre: Mover Imagen | Línea Blanca: Slider', fg_color=self.HUD_BG, text_color=self.HUD_TEXT, corner_radius=0, font=ctk.CTkFont(size=11))
