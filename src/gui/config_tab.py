...
# Internal filename: 'src\\gui\\config_tab.py'
...
from .tags_frame import TagsSection
class ConfigTab(ctk.CTkFrame):
    def __init__(self, master, app, *args, **kwargs):
        ...
        self.btn_tags = ctk.CTkButton(self.sidebar_frame, text='Etiquetas', fg_color='transparent', text_color=self.MENU_NORMAL_TEXT, hover_color=self.MENU_SELECTED_BG, anchor='w', command=lambda: self.select_section('tags'))
        self.btn_tags.grid(row=5, column=0, padx=10, pady=5, sticky='ew')
        ...
        self.btn_console.grid(row=6, column=0, padx=10, pady=5, sticky='ew')
        self.menu_buttons = {'general': self.btn_general, 'cookies': self.btn_cookies, 'deps': self.btn_deps, 'models': self.btn_models, 'tags': self.btn_tags, 'console': self.btn_console}
        self.update_cache = {}
    ...
    def _setup_sections(self):
        ...
        self.theme_display_names = {'blue': 'Azul (Estándar)', 'dark-blue': 'Azul Profundo', 'green': 'Verde (Estándar)'}
        self.theme_internal_names = {v: k for k, v in self.theme_display_names.items()}
        self.theme_menu = ctk.CTkOptionMenu(selection_group, values=['Azul (Estándar)', 'Azul Profundo'], command=self._on_theme_change, width=160)
        self.theme_menu.pack(side='left', padx=(10, 40))
        ...
        self.appearance_mode_menu = ctk.CTkOptionMenu(selection_group, values=['Sistema', 'Claro', 'Oscuro'], command=self._on_appearance_mode_change, width=120)
        ...
        self.import_theme_btn = ctk.CTkButton(buttons_row, text='Importar Tema', width=100, fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT, command=self._import_theme)
        self.import_theme_btn.pack(side='left', padx=5)
        self.install_url_btn = ctk.CTkButton(buttons_row, text='Instalar URL', width=100, fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT, command=self._install_theme_from_url)
        self.install_url_btn.pack(side='left', padx=5)
        self.view_template_btn = ctk.CTkButton(buttons_row, text='Ver Plantilla', width=100, fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT, command=self._on_view_template)
        self.view_template_btn.pack(side='left', padx=5)
        self.open_themes_btn = ctk.CTkButton(buttons_row, text='📁', width=40, fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT, command=self._open_themes_folder)
        self.open_themes_btn.pack(side='left', padx=5)
        self.delete_theme_btn = ctk.CTkButton(buttons_row, text='Borrar Tema', width=100, fg_color=self.CANCEL_BTN, hover_color=self.CANCEL_HOVER, text_color=self.CANCEL_TEXT, command=self._delete_theme)
        ...
        self.clear_vram_btn = ctk.CTkButton(vram_controls, text='Liberar VRAM Ahora', width=160, height=26, font=ctk.CTkFont(size=11, weight='bold'), fg_color=self.CANCEL_BTN, hover_color=self.CANCEL_HOVER, text_color=self.CANCEL_TEXT, command=self._manual_vram_clear)
        ...
        self.dpi_warning_label = ctk.CTkLabel(dpi_group, text='⚠️ Valores > 1200 DPI pueden agotar la RAM. Usar con precaución.', font=ctk.CTkFont(size=11, weight='bold'), text_color='#FF8C00', wraplength=550, justify='left')
        ...
        self.inkscape_path_entry = ctk.CTkEntry(ink_path_frame, placeholder_text='C:\\Program Files\\Inkscape')
        ...
        self.ink_download_btn = ctk.CTkButton(ink_status_row, text='Descargar Inkscape', width=130, height=26, fg_color=self.SECONDARY_BTN, hover_color=self.SECONDARY_HOVER, text_color=self.SECONDARY_TEXT, command=lambda: webbrowser.open(INKSCAPE_URL))
        self.ink_download_btn.pack(side='left', padx=(0, 10))
        self.ink_verify_btn = ctk.CTkButton(ink_status_row, text='Verificar Instalación', width=130, height=26, fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT, command=self._check_inkscape_status)
        ...
        self.clean_titles_switch = ctk.CTkSwitch(downloads_group, text='Limpieza automática de títulos (Eliminar emojis y caracteres especiales)', variable=self.clean_titles_var, command=self._on_title_cleanup_toggle)
        self.clean_titles_switch.pack(anchor='w')
        ctk.CTkLabel(downloads_group, text='Elimina emojis y símbolos para evitar errores en DaVinci Resolve y Adobe.', font=ctk.CTkFont(size=11), text_color='gray60').pack(anchor='w', padx=25, pady=(5, 0))
        ...
        self.cookie_mode_menu = ctk.CTkOptionMenu(mode_frame, values=['No usar', 'Archivo Manual...', 'Desde Navegador'], command=self.on_cookie_mode_change)
        self.cookie_mode_menu.pack(side='left', fill='x', expand=True, padx=(0, 10))
        self.test_cookies_btn = ctk.CTkButton(mode_frame, text='Probar Cookies', width=120, fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT, state='disabled', command=self._test_cookies)
        ...
        notice_text = '⚠️ Los navegadores Chromium (Chrome, Edge, Brave, Opera, Vivaldi) han implementado bloqueos de cifrado muy estrictos que impiden extraer sus cookies directamente.\n\n✅ Se recomienda usar Firefox (o sus variantes) ya que permiten una extracción directa de sesión más estable, o en su lugar optar por usar el modo superior: \'Archivo Manual\'. \n(Si igualmente ocurre un fallo durante la extracción, asegúrate de haber cerrado el navegador por completo primero).'
        ctk.CTkLabel(notice_frame, text=notice_text, font=ctk.CTkFont(size=11), text_color=('gray10', 'white'), justify='left', wraplength=520).pack(padx=15, pady=10, anchor='w')
        ...
        import webbrowser
        ctk.CTkButton(self.help_frame, text='Descargar \'Get cookies.txt LOCALLY\' (GitHub)', fg_color=('#3B8ED0', '#1F6AA5'), border_width=0, text_color='white', font=ctk.CTkFont(weight='bold'), command=lambda: webbrowser.open_new_tab('https://github.com/kairi003/Get-cookies.txt-LOCALLY')).pack(anchor='w', padx=20, pady=(0, 10))
        ...
        self.btn_check_all_updates = ctk.CTkButton(updatable_header_frame, text='Buscar Actualizaciones', width=150, fg_color=self.ANALYZE_BTN, hover_color=self.ANALYZE_HOVER, text_color=self.ANALYZE_TEXT, command=self.check_all_updates)
        ...
        self.recovery_label = ctk.CTkLabel(recovery_frame, text='Estas dependencias son necesarias para las Herramientas de Imagen y ya vienen pre-instaladas. Si notas que te faltan, fallan o se han corrompido, la opción recomendada es reinstalar el programa directamente desde la página oficial.', justify='left', text_color=('gray10', '#DCE4EE'), wraplength=550)
        ...
        self.btn_add_custom = ctk.CTkButton(custom_actions_bar, text='Añadir Modelo', fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT, height=28, command=self._on_add_custom_model_config)
        self.btn_add_custom.pack(side='left', padx=(0, 10))
        self.btn_delete_custom = ctk.CTkButton(custom_actions_bar, text='Borrar Seleccionados', fg_color=self.SECONDARY_BTN, hover_color=self.SECONDARY_HOVER, text_color=self.SECONDARY_TEXT, height=28, state='disabled', command=self._delete_selected_custom_models)
        ...
        self.custom_models_listbox = Listbox(list_container, bg=self._resolve_color(_lb_bg), fg=self._resolve_color(_lb_text), font=('Segoe UI', 10), selectmode=EXTENDED, borderwidth=1, highlightthickness=0, height=8)
        ...
        frame_tags = TagsSection(self.content_container, self.app, self)
        self.sections['tags'] = frame_tags
        frame_tags._scrollbar.bind('<B1-Motion>', lambda e: self.app.update_idletasks())
        frame_tags.bind('<MouseWheel>', lambda e: self.app.after(10, self.app.update_idletasks))
    def _setup_console_section(self):
        ...
        ctk.CTkLabel(left_frame, text='Consola de Diagnóstico', font=ctk.CTkFont(size=20, weight='bold')).pack(anchor='w')
        ...
        self._console_switch = ctk.CTkSwitch(controls_row, text='Activar Consola', font=ctk.CTkFont(size=12), variable=self._console_switch_var, onvalue=True, offvalue=False, command=self._on_console_switch_toggle)
        self._console_switch.pack(side='left', padx=(0, 20))
        self._console_wrap_switch = ctk.CTkSwitch(controls_row, text='Ajuste de línea', font=ctk.CTkFont(size=12), variable=self._console_wrap_var, onvalue=True, offvalue=False, command=self._on_console_wrap_toggle)
        ...
        self._btn_copy = ctk.CTkButton(log_btn_frame, text='Copiar', width=90, height=28, fg_color=('#1565C0', '#1565C0'), hover_color=('#0D47A1', '#0D47A1'), text_color='white', command=self._console_copy)
        self._btn_copy.pack(side='left', padx=3)
        self._btn_export = ctk.CTkButton(log_btn_frame, text='Exportar', width=90, height=28, fg_color=self.SECONDARY_BTN, hover_color=self.SECONDARY_HOVER, text_color=self.SECONDARY_TEXT, command=self._console_export)
        self._btn_export.pack(side='left', padx=3)
        self._btn_clear = ctk.CTkButton(log_btn_frame, text='Limpiar', width=90, height=28, fg_color=self.CANCEL_BTN, hover_color=self.CANCEL_HOVER, text_color=self.CANCEL_TEXT, command=self._console_clear)
        self._btn_clear.pack(side='left', padx=(3, 0))
        self._console_textbox = ctk.CTkTextbox(frame_console, font=ctk.CTkFont(family='Consolas', size=11), fg_color=self.CONSOLE_BG, text_color=self.CONSOLE_TEXT, wrap='word' if self._console_wrap_var.get() else 'none', state='disabled')
        self._console_textbox.grid(row=1, column=0, sticky='nsew')
        self._console_textbox.tag_config('user_command', foreground='#52A2F2')
        self._console_textbox.tag_config('error', foreground='#e74c3c')
        self._console_textbox.tag_config('warning', foreground='#f39c12')
        ...
        self._console_cmd_entry = ctk.CTkEntry(bottom_bar, placeholder_text='ffmpeg -version  ·  yt-dlp --help  ·  yt-dlp -f best URL', font=ctk.CTkFont(family='Consolas', size=11), state='disabled')
        ...
        self._btn_execute = ctk.CTkButton(cmd_btn_frame, text='Ejecutar', width=90, height=28, fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT, state='disabled', command=self._execute_console_command)
        self._btn_execute.pack(side='left', padx=(0, 3))
        self._btn_cancel_cmd = ctk.CTkButton(cmd_btn_frame, text='Cancelar', width=90, height=28, fg_color='transparent', border_width=1, border_color=self.CANCEL_BTN, text_color=self.CANCEL_TEXT, hover_color=self.CANCEL_HOVER, state='disabled', command=self._cancel_console_command)
    ...
    def append_to_console(self, text: str, tag: str = 'normal'):
        """Callback from ConsoleLogger/Handler. Adds text with timestamp and line limit."""
        try:
            ...
            # Handle carriage return (overwrite last line)
            if '\r' in text:
                parts = text.split('\r')
            ...
            for line in lines:
                # If we are at the position of a carriage return, delete current line
                ...
                # Prepend timestamp if line is not empty and doesn't already have one
                if (self._console_at_start and line.strip() and
                    not bool(re.match(r'^\s*\[\d{2}:\d{2}:\d{2}\]', line))):
            ...
            # Limit number of lines (cleanup every 3 seconds)
            now = time.time()
            if not hasattr(self, '_last_console_cleanup'):
                self._last_console_cleanup = 0
            if now - self._last_console_cleanup > 3.0:
                self._last_console_cleanup = now
                MAX_LINES = 2500
            ...
            # Log or handle silently; avoid crashing the GUI
            print(f"Error in append_to_console: {e}")
            return
    ...
    def _console_copy(self):
        ...
        try:
            ...
            self._btn_copy.configure(text='¡Copiado!', fg_color=self.STATUS_SUCCESS, hover_color=self.STATUS_SUCCESS, text_color='white')
    ...
    def _reset_copy_button(self):
        ...
        try:
            self._btn_copy.configure(text='Copiar', fg_color=self.SECONDARY_BTN, hover_color=self.SECONDARY_HOVER, text_color=self.SECONDARY_TEXT)
        except Exception:
            return None
            ...
            try:
                ...
                if val > 2400:
                    ...
                    if val < 70:
                        pass
                self.app.vector_dpi = val
                self.vector_dpi_var.set(min(val, 1200))
                self._update_dpi_warning(val)
                self.app.save_settings()
            except ValueError:
                return None
    ...
    def _import_theme(self):
        ...
        # Check if file already exists and ask for overwrite
        if os.path.exists(dest_path):
            if not messagebox.askyesno(
                'Sobrescribir',
                f'El tema \'{filename}\' ya existe. ¿Quieres sobrescribirlo?'
            ):
    ...
    def _install_theme_from_url(self):
        ...
        Tooltip.hide_all()  # Assume Tooltip is defined elsewhere

        dialog = URLInputDialog(
            master=self.app,
            text='Pega el link directo (JSON Raw) del tema:',
            title='Instalar Tema por URL'
        )
    ...
    def _delete_theme(self):
        ...
        if not internal_name:
            ...
            system_themes = ['blue', 'dark-blue', 'green', 'green_shrek', 'dorado', 'tokyo', 'coffee_noir', 'cyberpunk_neon', 'forest_moss', 'midnight_ocean', 'sunset_lavender', 'shrek']
    ...
    def _populate_rembg_model_rows(self):
        ...
        for family_name, models in REMBG_MODEL_FAMILIES.items():
            ...
            header = ctk.CTkLabel(header_frame, text=family_name, font=ctk.CTkFont(size=14, weight='bold'), text_color=self.SECTION_SUBTITLE)
            header.pack(side='left')
            self.config_subtitles.append(header)
            folder_icon = ctk.CTkButton(header_frame, text='Abrir Carpeta', width=100, height=24, fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT, font=ctk.CTkFont(size=11, weight='bold'), command=lambda f=family_name: self._open_family_folder(f))
            folder_icon.pack(side='right')
            self.model_family_icons.append(folder_icon)
            if family_name == RMBG2_FAMILY:
                notice = 'Únicamente el modelo \'Automático\' puede descargarse directamente desde DowP. Para descargar los demás formatos especializados es requisito obligatorio tener una cuenta conectada en Hugging Face. Una vez descargados manualmente desde su web, colócalos en la carpeta que se muestra al pulsar \'Abrir Carpeta\'.'
                ...
                text_lbl = ctk.CTkLabel(notice_frame, text=notice, font=ctk.CTkFont(size=11), text_color=('gray10', 'white'), justify='left', wraplength=520)
                text_lbl.pack(anchor='w', padx=15, pady=(10, 3))
                self.huggingface_btn = ctk.CTkButton(notice_frame, text='Explorar Repositorio Web (HuggingFace)', font=ctk.CTkFont(size=11, weight='bold'), fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT, width=250, command=lambda: __import__('webbrowser').open('https://huggingface.co/briaai/RMBG-2.0/tree/main/onnx'))
            ...
            for model_name, model_info in models.items():
                ...
                self._create_model_row(parent=self.rembg_models_frame, row_key=row_key, display_name=model_name, description=f"Archivo: {model_info['file']}", model_info=model_info, kind='rembg', manual_web=is_hf_manual, show_folder_btn=False)
    def _populate_upscaling_model_rows(self):
        ...
        for tool_name, tool_info in UPSCALING_TOOLS.items():
            ...
            self._create_model_row(parent=self.upscaling_models_frame, row_key=row_key, display_name=tool_name, description=f"Ejecutable: {tool_info['exe']}", model_info=tool_info, kind='upscaling', show_folder_btn=True)
    def _create_model_row(self, parent, row_key, display_name, description, model_info, kind, manual_web=False, show_folder_btn=True):
        ...
        dl_btn = ctk.CTkButton(btn_frame, text=dl_label, width=100, fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT)
        dl_btn.pack(side='left', padx=2)
        folder_btn = None
        if show_folder_btn:
            folder_btn = ctk.CTkButton(btn_frame, text='Carpeta', width=80, fg_color=self.SECONDARY_BTN, hover_color=self.SECONDARY_HOVER, text_color=self.SECONDARY_TEXT, font=ctk.CTkFont(size=11, weight='bold'))
            folder_btn.pack(side='left', padx=2)
        del_btn = ctk.CTkButton(btn_frame, text='Eliminar', width=80, fg_color=self.CANCEL_BTN, hover_color=self.CANCEL_HOVER, text_color=self.CANCEL_TEXT)
        del_btn.pack(side='left', padx=2)
        self.model_rows[row_key] = {'status_lbl': status_lbl, 'pct_lbl': pct_lbl, 'dl_btn': dl_btn, 'del_btn': del_btn, 'folder_btn': folder_btn, 'model_info': model_info, 'kind': kind, 'manual_web': manual_web}
    ...
    def _refresh_model_row(self, row_key):
        ...
        if row_key not in self.model_rows:
            ...
            if os.path.exists(path):
                ...
                row['dl_btn'].configure(state='disabled', text='Instalado', fg_color=self.SECONDARY_BTN, text_color=self.SECONDARY_TEXT)
                row['del_btn'].configure(state='normal', fg_color=self.CANCEL_BTN, hover_color=self.CANCEL_HOVER, text_color=self.CANCEL_TEXT, border_width=0)
            else:
                row['status_lbl'].configure(text='No descargado', text_color='gray50')
                row['pct_lbl'].configure(text='')
                row['dl_btn'].configure(state='normal', text='Descargar', fg_color=self.DOWNLOAD_BTN, hover_color=self.DOWNLOAD_HOVER, text_color=self.DOWNLOAD_TEXT)
                row['del_btn'].configure(state='disabled', fg_color='transparent', border_width=1, text_color=('gray10', 'gray90'))
            if row.get('folder_btn'):
                row['folder_btn'].configure(fg_color=self.TERTIARY_BTN, hover_color=self.TERTIARY_HOVER, text_color=self.TERTIARY_TEXT)
    def _download_model(self, row_key):
        """Inicia la descarga de un modelo en un hilo separado."""
        # Ensure required imports are available (ideally at module level)
        import os
        import threading
        import webbrowser
        import requests
        from tkinter import messagebox  # optional for errors
        ...
        # Handle Hugging Face or missing URL
        if not url:
            messagebox.showerror('Error', 'No se ha especificado una URL para este modelo.')
            return
        if 'huggingface.co' in url:
            webbrowser.open(url)
            return  # do not download, just open browser

        # Prepare UI for download
        row['dl_btn'].configure(state='disabled', text='Descargando...')
        row['status_lbl'].configure(text='0%', text_color='#1f6aa5')
        row['pct_lbl'].configure(text='')

        def do_download():
            """Función que corre en un hilo para descargar el archivo."""
            try:
                ...
                # Download successful: refresh the row and update buttons
                self.app.after(0, lambda: self._refresh_model_row(row_key))
                self.app.after(0, lambda: row['dl_btn'].configure(state='disabled', text='Instalado'))

            except Exception as e:
                # Error handling: show error and re-enable the button
                error_msg = f'Error: {str(e)[:60]}'
                self.app.after(0, lambda: row['status_lbl'].configure(text=error_msg, text_color='red'))
    ...
    def _download_upscaling_tool(self, row_key, tool_info, dest_exe_path, url):
        ...
        import threading
        from tkinter import messagebox  # optional for additional error feedback
        ...
        # Find the engine key that matches the given tool info
        engine_key = None
        ...
        if not engine_key:
            # If engine not recognized, show error and re-enable button
            row['status_lbl'].configure(text='Error: Motor no reconocido', text_color='red')
            row['dl_btn'].configure(state='normal', text='Reintentar')
            messagebox.showerror('Error', 'Motor no reconocido en la configuración.')
            return

        def do_download():
            """Función que corre en un hilo para descargar el upscaling tool."""
            try:
                # Progress callback (runs in the background thread)
                ...
                if success:
                    # Refresh the row on success (updates status to installed)
                    self.app.after(0, lambda: self._refresh_model_row(row_key))
                    self.app.after(0, lambda: row['dl_btn'].configure(state='disabled', text='Instalado'))
                else:
                    # If the function returns False (but didn't raise), treat as failure
                    raise Exception('Fallo en la descarga o extracción.')

            except Exception as e:
                # Handle any error: update UI and re-enable retry button
                error_msg = str(e)[:60]
                self.app.after(0, lambda: row['status_lbl'].configure(text='Error', text_color='red'))
                self.app.after(0, lambda: row['pct_lbl'].configure(text=error_msg))
                self.app.after(0, lambda: row['dl_btn'].configure(state='normal', text='Reintentar'))
    ...
    def _create_dependency_row(self, parent, name, description, key):
        ...
        if key == 'ffmpeg':
            safe_btn = ctk.CTkButton(btn_container, text='Restaurar (8.0.1)', width=140, fg_color=self.STATUS_SUCCESS, command=self.manual_ffmpeg_safe_update_check)
    ...
    def _create_fixed_dependency_row(self, parent, name, description, key, url):
        ...
        btn_container.grid(row=0, column=2, sticky='nse', padx=(10, 0))
        import webbrowser
        btn = ctk.CTkButton(btn_container, text='Sitio Web Oficial', width=140, fg_color='transparent', border_width=1, text_color=('gray10', 'gray90'), command=lambda u=url: webbrowser.open(u))
        btn.pack(side='top', pady=2, fill='x')
        self.dep_buttons[key] = btn
    ...
    def refresh_theme(self):
        ...
        for card in self.config_cards:
            if card.winfo_exists():
                ...
                if card in [self.appearance_frame, self.master_frame, self.updatable_frame, self.fixed_frame, self.rembg_models_frame, self.upscaling_models_frame, self.custom_models_mgr_frame]:
        ...
        main_btns = ['ink_verify_btn', 'test_cookies_btn', 'btn_check_all_updates', 'btn_add_custom', '_btn_execute']
        ...
        sec_btns = ['ink_download_btn', 'btn_delete_custom', '_btn_copy', '_btn_export']
        ...
        if hasattr(self, 'custom_models_listbox') and self.custom_models_listbox.winfo_exists():
                self.custom_models_listbox.configure(bg=self._resolve_color(self.app.get_theme_color('LISTBOX_BG', ['#F9F9FA', '#18181A'])), fg=self._resolve_color(self.app.get_theme_color('LISTBOX_TEXT', ['gray10', '#DCE4EE'])))
        ...
        if hasattr(self, 'custom_models_listbox'):
            ...
            self.custom_models_listbox.configure(bg=_bg, fg=_fg, selectbackground=_sbg, selectforeground=_sfg, highlightbackground=_bdc, highlightthickness=1, borderwidth=0)
        if hasattr(self, '_console_textbox'):
            _lb_bg = self.app.get_theme_color('LISTBOX_BG', ['#F9F9FA', '#18181A'])
            _lb_text = self.app.get_theme_color('LISTBOX_TEXT', ['gray10', '#DCE4EE'])
            ...
            self._console_textbox.configure(fg_color=self._resolve_color(_lb_bg), text_color=self._resolve_color(_lb_text))
    ...
    def _load_local_versions(self):
        ...
        # ***<module>.ConfigTab._load_local_versions: Failure: Different control flow
        ...
        versions = {'ffmpeg': env_status.get('local_version') or 'No encontrado', 'deno': env_status.get('local_deno_version') or 'No encontrado', 'poppler': env_status.get('local_poppler_version') or 'No encontrado', 'ytdlp': env_status.get('local_ytdlp_version') or 'No encontrado'}
        ...
        if versions['ffmpeg'] == '8.0.1':
            if 'ffmpeg_safe' in self.dep_buttons:
    ...
    def check_all_updates(self):
        ...
        self.btn_check_all_updates.configure(state='disabled', text='Buscando...', fg_color=self.STATUS_PENDING)
    ...
    def _on_all_updates_check_complete(self, env_status):
        ...
        self.btn_check_all_updates.configure(state='normal', text='Buscar Actualizaciones', fg_color=self.ANALYZE_BTN, hover_color=self.ANALYZE_HOVER, text_color=self.ANALYZE_TEXT)
        import re
        ...
        if local_ffmpeg != 'No encontrado' and latest_ffmpeg:
            try:
                local_v = version.parse(re.search('v?(\\d+\\.\\d+(\\.\\d+)?)', local_ffmpeg).group(1))
                latest_v = version.parse(re.search('v?(\\d+\\.\\d+(\\.\\d+)?)', latest_ffmpeg).group(1))
                if latest_v > local_v:
        ...
        if update_available_ffmpeg:
            ...
            self.dep_buttons['ffmpeg'].configure(state='normal', text='Actualizar', fg_color=self.DOWNLOAD_BTN, text_color=self.DOWNLOAD_TEXT, hover_color=self.DOWNLOAD_HOVER)
        else:
        ...
        if update_available_deno:
            ...
            self.dep_buttons['deno'].configure(state='normal', text='Actualizar', fg_color=self.DOWNLOAD_BTN, text_color=self.DOWNLOAD_TEXT, hover_color=self.DOWNLOAD_HOVER)
        else:
            self.dep_labels['deno'].configure(text=f'Versión: {local_deno} \n(Actualizado)', text_color=self.MENU_NORMAL_TEXT)
        ...
        if update_available_poppler:
            ...
            self.dep_buttons['poppler'].configure(state='normal', text='Actualizar', fg_color=self.DOWNLOAD_BTN, text_color=self.DOWNLOAD_TEXT, hover_color=self.DOWNLOAD_HOVER)
        else:
            self.dep_labels['poppler'].configure(text=f'Versión: {local_poppler} \n(Actualizado)', text_color=self.MENU_NORMAL_TEXT)
        ...
        if update_available_ytdlp:
            ...
            self.dep_buttons['ytdlp'].configure(state='normal', text='Actualizar', fg_color=self.DOWNLOAD_BTN, text_color=self.DOWNLOAD_TEXT, hover_color=self.DOWNLOAD_HOVER)
        else:
            self.dep_labels['ytdlp'].configure(text=f'Versión: {local_ytdlp} \n(Actualizado)', text_color=self.MENU_NORMAL_TEXT)
    ...
    def download_ffmpeg_update(self):
        ...
        if not response:
            ...
            def download_task():
                success = download_and_install_ffmpeg(self.latest_ffmpeg_version, self.latest_ffmpeg_url, lambda text, val, c_mb=0, t_mb=0: self.app.after(0, lambda: self.update_setup_download_progress('ffmpeg', text, val)))
    ...
    def download_deno_update(self):
        ...
        def download_task():
            success = download_and_install_deno(self.latest_deno_version, self.latest_deno_url, lambda text, val, c_mb=0, t_mb=0: self.app.after(0, lambda: self.update_setup_download_progress('deno', text, val)))
    ...
    def download_poppler_update(self):
        ...
        def download_task():
            success = download_and_install_poppler(self.latest_poppler_version, self.latest_poppler_url, lambda text, val, c_mb=0, t_mb=0: self.app.after(0, lambda: self.update_setup_download_progress('poppler', text, val)))
    ...
    def download_ytdlp_update(self):
        ...
        def download_task():
            success = download_and_install_ytdlp(self.latest_ytdlp_version, self.latest_ytdlp_url, lambda text, val, c_mb=0, t_mb=0: self.app.after(0, lambda: self.update_setup_download_progress('ytdlp', text, val)))
    ...
    def manual_ffmpeg_safe_update_check(self):
        ...
        if not confirm:
            ...
            def download_task():
                tag, url = get_safe_ffmpeg_info(lambda text, val, c_mb=0, t_mb=0: self.app.after(0, lambda: self.update_setup_download_progress('ffmpeg', text, val)))
                success = download_and_install_ffmpeg(tag, url, lambda text, val, c_mb=0, t_mb=0: self.app.after(0, lambda: self.update_setup_download_progress('ffmpeg', text, val)))
                self.app.after(0, lambda: self._on_download_complete('ffmpeg', success))
            threading.Thread(target=download_task, daemon=True).start()
    def _on_download_complete(self, key, success):
        ...
        # Update progress UI (hide progress indicator)
        self.update_setup_download_progress(key, '', -1)
        ...
        if success:
            ...
            new_ver = env_status.get(mapping.get(key), 'Desconocida')

            # Update label and button for the component
            self.dep_labels[key].configure(text=f'Versión: {new_ver} \n(Actualizado)', text_color='gray50')
            self.dep_buttons[key].configure(
                state='disabled',
                text='Actualizado',
                fg_color=self.STATUS_PENDING,
                text_color=self.MENU_NORMAL_TEXT
            )

            # Clear update cache for this component
            if key in self.update_cache:
                self.update_cache[key]['update_available'] = False

            # Special handling for ffmpeg: enable the safe restore button if needed
            ...
            # Special handling for yt-dlp: ask for restart
            if key == 'ytdlp':
                Tooltip.hide_all()
            ...
            messagebox.showerror(
                'Error',
                f'La actualización de {key} falló. Revisa la consola o tu conexión.'
            )

        # Clean up lock file if it exists (used to prevent concurrent updates)
    ...
    def _run_cookie_test_thread(self, ydl_opts):
        ...
        try:
            ...
            if 'database is locked' in err_msg.lower() or 'database is locked' in full_trace.lower() or 'permission' in err_msg.lower() or ('file is locked' in full_trace.lower()):
                ...
                if 'could not find' in err_msg.lower():
            ...
            import re
            clean_err = re.sub('\\x1b\\[[0-9;]*m', '', err_msg)
        ...
        if not hasattr(self, 'custom_models_listbox'):
            ...
            for real_name, nickname in custom_models.items():
                ...
                if os.path.exists(bin_p):
                    total_size += os.path.getsize(bin_p)
                if os.path.exists(param_p):
                    total_size += os.path.getsize(param_p)
    ...
    def scroll_to_integrations(self):
        ...
        if 'general' in self.sections:
            ...
            try:
                self.update_idletasks()  # Ensure UI is updated before scrolling
                # Attempt to scroll to the bottom of the canvas (where integrations are)
